...
class Organizer(Auth):
    ...
    def base_path(self):
        return 'G2W/rest/organizers/%s' % self.key

    def headers(self):
        return {'Authorization': 'OAuth oauth_token=%s' % self.oauth}
    ...
    @property
    def past_webinars(self):
        return self._past_ex.result

    @property
    def future_webinars(self):
        return self._future_ex.result

    @property
    def sessioned_webinars(self):
        return self._sessioned_ex.result
        ...
        for ex in Exchange.async_exchange([self._future_ex, self._past_ex, self._sessioned_ex]):
            for that in ex.result:
                ...
                if this:
                    ...
                    webinars[that.key] = that
        return sort(webinars.values())

    @cached_property
    def _past_ex(self):
        return GetPastWebinars(self, self.starts_at, self.now - GTW_NOW_DELTA)

    @cached_property
    def _future_ex(self):
        return GetFutureWebinars(self)

    @cached_property
    def _sessioned_ex(self):
        return GetSessionedWebinars(self, self.starts_at, self.now - GTW_NOW_DELTA)
...
class GetSessionedWebinars(TimeWindowExchange):
    ...
    def params(self):
        return {'fromTime': self.start_at.strftime(API_TIME_FORMAT), 'toTime': self.end_at.strftime(API_TIME_FORMAT)}
