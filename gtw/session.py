...
class Session(Base, mixins.Session):
    ...
    def __repr__(self):
        return "Session(%s)" % kwargs_str(self, attrs=['key', '_starts_at', '_ends_at', '_started_at', '_ended_at', 'attendant_count'],
                                          cached=['attendees'])
    ...
    def __unicode__(self):
        ...
        session_key, webinar_key, subject, starts_at, ends_at, started_at, ended_at, self.timezone, count, registrants)

    def duration(self):
        return self.started_at and self.ended_at and self.started_at - self.ended_at

    @cached_property
    def _attendees_ex(self):
        return GetAttendees(self)

    @cached_property
    def attendees(self):
        return self.key and self._attendees_ex.result
    ...
    def __cmp__(self, other):
        return self._cmp_components(other, 'starts_at', 'ends_at', 'started_at', 'ended_at', 'key', 'attendant_count',
                                    cached_key('attendees'))

    @property  # ghetto webinar/session key -- useful for future events that don't have a session key yet
    def universal_key(self):
        if not self.webinar or not self.webinar.key or not is_cached(self.webinar,
                                                                     'sessions') or self not in self.webinar.sessions: return None
