...
class Webinar(Base):
    ...
    def __cmp__(self, other):
        return self._cmp_components(other, cached_key('sessions'), 'key', 'subject', 'description', 'timezone', cached_key('registrations'))
    ...
    @cached_property
    def sessions(self):
        return self._sessions_ex.result
    ...
    def registrants(self):
        ...
        for r in self.registrations:
            ...
            uniques.setdefault(r.email, {})[
                r.status if r.status in ('APPROVED', 'WAITING', 'UNREGISTERED') else 'OTHER'] = r  # future proofing a bit
    ...
    @cached_property
    def _sessions_ex(self):
        return GetSessions(self)

    @cached_property
    def _registrations_ex(self):
        return GetRegistrations(self)
...
class GetSessions(WebinarExchange):
    def sub_path(self):
        return 'webinars/%s/sessions' % self.webinar.key
