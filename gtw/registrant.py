...
class Registrant(Base):
    def __init__(self, **kwargs):
        ...
        if not self.viewings and (kwargs.get('duration') or kwargs.get(
                'attendanceTimeInSeconds')) and self.session and self.session.key and self.session.started_at:
    ...
    @property
    def duration(self):
        return self.viewings and (self.viewings[-1][-1] - self.viewings[0][0]) or None

    def merge(self, other):
        self._merge_primitives(other, 'webinar', 'session', 'key', 'email', 'first_name', 'last_name', 'registered_at', 'join_url',
                               'status')
        viewings = []
    ...
    def __cmp__(self, other):
        return self._cmp_components(other, 'email', 'last_name', 'first_name', 'started_at', 'ended_at', 'key', 'registered_at', 'join_url',
                                    'status', 'viewings')

    def clone(self):
        return Registrant().merge(self)

    def create(self):
        return self._create_ex.result

    @classmethod
    def _create(kls, registrants):
        return CreateRegistrant.bulk_results([r._create_ex for r in registrants], async=False)  # otherwise we api rate limits

    @cached_property
    def _create_ex(self):
        return CreateRegistrant(self)
    ...
    def __unicode__(self):
        started_at = self.started_at and "%s +%s" % (
            time(self.started_at.us, self.timezone).strftime('%m/%d/%y %I:%M%p').lower(), self.timezone) or '?'
        ended_at = self.ended_at and "%s +%s" % (
            time(self.ended_at.us, self.timezone).strftime('%m/%d/%y %I:%M%p').lower(), self.timezone) or '?'
        registered_at = self.registered_at and "%s +%s" % (
            time(self.registered_at.us, self.timezone).strftime('%m/%d/%y %I:%M%p').lower(), self.timezone) or '?'
        return u"%s[%s] %s (%s %s) %s [ %s ] (%s - %s) :%s =%s" % (
            self.started_at and 'A' or 'R', self.key, self.email, self.first_name, self.last_name, self.duration, self.status or '?',
            started_at, ended_at, registered_at, self.join_url)

    @property
    def timezone(self):
        return self.session and self.session.timezone or self.webinar and self.webinar.timezone
